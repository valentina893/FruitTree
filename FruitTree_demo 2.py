...
def screen3():
    ...
    user_hp = ttk.Progressbar(user_frame, orient='horizontal', mode='determinate', length=150, value=user_dict["health"])
    attack = Button(user_frame, text="attack", fg="blue", command=lambda: (damage_move(user_dict["fruit_dict"]), message.configure(text=f"waiting for {enemy_dict['name']}...")))
    ...
    enemy_hp = ttk.Progressbar(root, orient="horizontal", mode="determinate", length=300, value=enemy_dict["health"])
...
def host_server(username, port): # FIXME: way to avoid blocking network search?
    ...
    global user_dict, server, clientsocket
    ...
    if input_check(username, port, user_dict, curr_widgets[6], curr_widgets[7], curr_widgets[9]) == False:
        ...
        #curr_widgets[1].configure(text="waiting for joining player")
                ...
                print(is_socket_connected(server))
...
def is_socket_connected(sock): # CHATGPT
    try:
        # Try sending a small packet of data
        sock.send(b'\0')
    except socket.error:
        # An error occurred, meaning the connection is probably closed
        return False
    return True
...
def damage_move(move_set):
    ...
    curr_widgets[2].configure(fg="black", state=DISABLED)
    curr_widgets[3].configure(fg="black", command=DISABLED)

    miss_chance = random.choice(range(move_set["miss"][0], move_set["miss"][-1]+1))
    if miss_chance != 1:
        move_outcome["miss"] = False
        crit_chance = random.choice(range(move_set["crit"][0], move_set["crit"][-1]+1))
        if crit_chance != 1:
            move_outcome["crit"] = False
            move_outcome["total"] += 10
            print(move_outcome)
        else:
            move_outcome["crit"] = True
            move_outcome["total"] += 20
            print(move_outcome)
    else:
        move_outcome["miss"] = True
        print(move_outcome)
    
    
    if user_dict["role"] == "host":

        status_updateHost(move_outcome)

    else:
        
        status_updateJoin(move_outcome)
...
def user_move():
    ...
    Followed by: if statement to call recv_move() (when move_outcome is filled) which listens for what enemy uses and the stats of the move. game stats are then affected.
    '''
    ...
    #status_update(move_outcome)

def status_updateHost(dct):
    '''
    Function: sends moves to and receives oppenets moves via socket and updates all current game data and front end.
    Activated by: While loop in battle_sequence function.
    Followed by: Check to see if user has lost the battle. if so, socket sends victory message to opposing socket.
    '''

    global server, user_dict, enemy_dict

    '''server = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    server.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    server.bind((socket.gethostname(), user_dict["port"]))
    server.listen(1)
    clientsocket, address = server.accept()'''

    print ('status update HOST has been called')
    ...
    while listening:
        ...
        try:
            recv_pickle_move = clientsocket.recv(200)
            sent_move = pickle.dumps(dct)
            clientsocket.sendall(sent_move)
            recv_move = pickle.loads(recv_pickle_move)
        except:
            None
        else:
            listening = False
    ...
    print(f"{enemy_dict['name']} sent", recv_move)

    curr_widgets[2].configure(fg="red", state=NORMAL)
    curr_widgets[3].configure(fg="blue", state=NORMAL)

    user_dict["health"] -= recv_move["total"]
    enemy_dict["health"] -= dct["total"]
    curr_widgets[7].configure(value=enemy_dict["health"])
    curr_widgets[1].configure(value=user_dict["health"])

    if move_outcome["miss"] == False:
        move_outcome["crit"] = None
        move_outcome["miss"] = None
        move_outcome["total"] = 0
    else:
        move_outcome["miss"] = None

    print(enemy_dict["health"])



def status_updateJoin(dct):

    global server, user_dict, enemy_dict

    '''server = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    server.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    server.connect((socket.gethostname(), user_dict["port"]))'''

    print ('status update JOIN has been called')
    ...
    while listening:
        ...
        try:
            ...
            sent_move = pickle.dumps(dct)
            server.sendall(sent_move)
            recv_pickle_move = server.recv(200)
            recv_move = pickle.loads(recv_pickle_move)
        except:
            pass
        else:
