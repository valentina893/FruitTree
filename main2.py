...
def m_special(user_dct, en_dct):

    if user_dct['stats'][-3] == False:
        original_dct = copy.deepcopy(user_dct)
        new_dct = copy.deepcopy(en_dct)
        user_dct.clear()
        user_dct.update(copy.deepcopy(new_dct))
        user_dct['stats'][-2] = m_special
        user_dct['stats'][-1] = original_dct
        t1()
        print('mango has morphed into', en_dct['stats'][2])
    elif user_dct['stats'][-3] == True:
        original = user_dct['stats'][-1]
        user_dct.clear()
        user_dct.update(copy.deepcopy(original))
        t1()
        print("mango's morph special has ended.")
...
def p1_reg_move():
    ...
    print(p1c_dict['stats'][2], ':', p1c_dict[list(p1c_dict.keys())[0]][-1], p1c_dict[list(p1c_dict.keys())[1]][-1], '\nhp & power:', p1c_dict['stats'][0], p1c_dict['stats'][1])
    print('------------------')

    u_inp = input().lower()

    if u_inp == list(p1c_dict.keys())[0]: # regular move
        if p1c_dict[list(p1c_dict.keys())[0]][0] > 0:
            p1c_dict[list(p1c_dict.keys())[0]][0] -= 1
            return (p1c_dict[list(p1c_dict.keys())[0]], p1c_dict)
        else:
            return (False, p1c_dict)
    elif u_inp == list(p1c_dict.keys())[1]:
        if p1c_dict['stats'][1] > 0:
            p1c_dict['stats'][1] -= 1
            return (p1c_dict[list(p1c_dict.keys())[1]], p1c_dict)
        else:
            return (False, p1c_dict)
    else:
        return (False, p1c_dict)
...
# should ai be smart enough to not choose useless attacks? --> ai will be stupid for now
    en_inp = random.choice([1, 2, 3])

    return ([1, 2, 3, 4, 5], p2c_dict)
...
def combat(): # takes player choices as values, flips coin to see who goes first using values, uses functions to manipulate health and stats
    ...
    if p1[0] != False:
        if p1[0][-2] == 'regular':
            ...
            if p1c_dict['stats'][-3] == True:
                p1c_dict['stats'][-4] += 1
        else:
            t1()
            print('player 1 used a special move:', p1[0][-1])
            if p1c_dict['stats'][-3] == False:
                p1c_dict['stats'][3](p1c_dict, p2c_dict)
                p1c_dict['stats'][-3] = True
            else:
                p1c_dict['stats'][-4] += 1
            t1()
            print('player 2 used', p2[0][-1])
    ...
    if p1c_dict['stats'][-4] == 5:
        p1c_dict['stats'][-2](p1c_dict, p2c_dict)
        p1c_dict['stats'][-3] = False
...
mango = { # name : pp, damage, speed-weight, crit, miss, call
           ...
           'm':[None, 0, 0.3, None, None, 'special', 'morph'],
           'stats':[100, 1, 'mango', m_special, 0, False, None, None],
          } # health, power, name, specials * 3, special_cnt, special_bool, reversal_foo_indicator

banana = {
           'k':[10, 10, 0.5, (1,6), (1,8), 'regular', 'kick'],
           's':[5, 20, 0.2, (3,6), (2,8), 'special', 'shoot'],
           'stats':[100, 1, 'banana', b_special, 0, False, None, None]
          }

strawberry = {
            'p':[10, 10, 0.5, (1,6), (1,8), 'regular', 'pound'],
            'x':[5, 0, 0.3, None, None, 'special', 'x'], #healing ability
            'stats':[100, 1, 'strawberry', s_special, 0, False, None, None]
        }
...
if g_mode_choice[1] == 'r': # chooses characters if in regular mode
    while len(char_choice1) < 1:
        ...
        if g_mode_choice[0] == 2:
            ...
            p2_char1 = random.choice(list(char_sel.keys())[1:])
            char_choice2.append(p2_char1)
